...
let objAbc = localStorage.getItem('data');
if (objAbc != null) {
...
function ShowItems() {
    ...
    userArray.forEach((data, i) => {
        statement += `<tr>
    <th><span class="sno"><b>${i+1}</b>.</span> <input type="checkbox" class="xyz" id="check${i}" onclick="done(${i})"/></th>
    <td> <span class=" h5" id="text${i}"> ${data.name} </span> </td>
    <td> 
    <button class=" btn btn-danger" style="color:white; background:rgb(185, 66, 66);" onclick="deleteList(${i})">Delete</button>
    <button class=" btn btn-primary" style="color:blue; background:rgb(127, 166, 221);" onclick="editList(${i})">Edit</button>
    </td>
  </tr>`;
        i++;
    })
